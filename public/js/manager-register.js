...
form?.addEventListener('submit', async event => {
  ...
  try {
    ...
    const sessionScope = sharedHost ? (tenantSlug || 'blockerino') : `host:${location.hostname.toLowerCase()}`;
    localStorage.setItem(`token:${sessionScope}`, data.token);
    if (sharedHost && tenantSlug === 'blockerino') localStorage.setItem('token', data.token);
    localStorage.setItem('user', JSON.stringify(data.user || {}));
    showMessage('Conta criada. Abrindo seu painel…', 'success');
    const tenantQuery = sharedHost && tenantSlug !== 'blockerino' ? `?tenant=${encodeURIComponent(tenantSlug)}` : '';
    window.setTimeout(() => { window.location.href = `/manager/${tenantQuery}`; }, 500);
  } catch (error) {
