...
async function loadTenant(slug, host) {
  ...
  let tenant = null;
  if (slug === DEFAULT_TENANT_ID || (!slug && ['block777-omega.vercel.app', 'localhost', '127.0.0.1', ''].includes(host))) {
...
export async function tenantContext(req, res, next) {
  try {
    const slug = normalizeTenantSlug(req.headers['x-tenant-slug'] || req.query.tenant || req.body?.tenant_slug);
    const tenant = await loadTenant(slug, requestHost(req));
